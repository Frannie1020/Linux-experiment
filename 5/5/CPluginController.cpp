...
bool CPluginController::Process(char *Function)//插件存在的判断标志
{
	...
	{
		if (strcmp(Function,m_vpPlugin[i]->GetPluginNames())==0)//获取文件名意外失败，直接退出
			break;
	}
...
bool CPluginController::ProcessFunc(char *Function, char *FileNames) //插件命令执行
{
	for (unsigned int i = 0; i < m_vpPlugin.size(); i++)//遍历插件文件名
	{
		if (strcmp(Function, m_vpPlugin[i]->GetPluginNames())== 0)
		{
			m_vpPlugin[i]-> Function(FileNames);//获取文件名意外失败，直接退出
				break;
		}
	}
	return true;

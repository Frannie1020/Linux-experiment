...
public:
	...
	virtual void Print()//打印函数的内容，提示用户输入L+文件名可以获取文件行数
	{
	...
	{
		cout << "Func NUM " << FUNC_NUM << ":this function will knows the doc's lines" << endl;
	}
	...
	virtual char *GetPluginNames()//获取插件文件名，输入L加空格以后即可
	{
		return LINE;
	}
